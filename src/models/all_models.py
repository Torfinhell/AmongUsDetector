...
from src.utils import calculate_norm_grad, drawbboxes, nms
...
class MyModel(L.LightningModule):
    def __init__(self, cfg: ModelTrainConfig):
        ...
        self.mAP_score = {
            "generated": PRauc(self.metrics_cfg),
            "test": PRauc(self.metrics_cfg),
        }
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx: int = 0):
        return self._step(
            batch, f"val_{self.dataloader_names[dataloader_idx]}", batch_idx
        )
    ...
    def on_train_epoch_end(self):
        ...
        for loss_kind in self.losses_names:
            ...
            if avg_loss is not None:
                print(
                    f"Epoch {self.current_epoch} - {loss_kind} Train Loss: {avg_loss.item():.6f}"
                )
    ...
    def on_validation_epoch_end(self):
        ...
        for name_val_ds in self.dataloader_names.values():
            for loss_kind in self.losses_names:
                avg_loss = self.trainer.callback_metrics.get(
                    f"val_{name_val_ds}/{loss_kind}", None
                )
                if avg_loss is not None:
                    print(
                        f"Epoch {self.current_epoch} - {loss_kind} Val_{name_val_ds} Loss: {avg_loss.item():.6f}"
                    )
        ...
        if kind.startswith("val"):
            ...
            if self.batch_val_show and batch_idx == 0:
                self.logger.experiment.add_images(
                    f"{kind}_images",
                    torch.stack(drawbboxes(batch["image"], preds)),
