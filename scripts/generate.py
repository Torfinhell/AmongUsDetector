...
def generate(destination_folder, background_folder=None, num_generations=10,
num_figures=3, augment=False, random_color=False, draw_bbox=False,
    ...
    with open(csv_file, "w", newline="") as f_csv:
        ...
        for id in range(num_generations):
            ...
            for _ in range(num_figures):
                ...
                visors = [random_hex_color() for _ in range(3)] if random_color else ["#FFFFFF"]*3
                among_us = AmongUs(width=width, height=height, body_color=body, visor_colors=visors)

                fg_bgr, fg_alpha = svg_to_cv2_image(among_us, among_us.width, among_us.height)
