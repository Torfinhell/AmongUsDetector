...
#     name="inference_qwen"
# )
# def inference_qwen(input_folder, output_folder: str = "", max_new_tokens: int = 1024, yandex_token:Optional[str]=None):
#     '''
...
#     input_csv=pd.read_csv(input_folder)
#     with CsvChunkDownloader()
#     for id, file_name,video_name,extracted_text,game_state,is_imposter,face_id in input_csv.iterrows():
...
#             color_name = center_pixel_class(image[xmin:xmax, ymin:ymax, :])

#         old_df = pd.read_csv(csv_path)
#         final_df = pd.concat([new_rows, old_df], ignore_index=True)


# def center_pixel_class(image):
#     h, w, d = image.shape
#     assert d == 3
